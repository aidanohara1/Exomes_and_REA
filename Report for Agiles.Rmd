...
AGiles <- AGiles %>% mutate(across(c(Reanalyed,
                                     Reclassified,
                                     Original.Latest,
                                     Report.Outcome,
...
AGiles_OG <- AGiles_OG %>% mutate(across(c(Reanalyed,
                                           Reclassified,
                                           Original.Latest,
                                           Report.Outcome,
                                           Initiator.of.reanalysis,
...
justReanaz <- AGiles %>% filter(Reanalyed == 1)
...
>>>>>>> b04ff2aecfe8244bd3609451d9e362fe5df7e08d
...
model3 <- glm(data = originals,formula = classification ~ abcBuckets + sex + age + indication,family = binomial)

```

```{r gilesmodel4}

model4 <- glm(data = originals,formula = reclassification ~ abcBuckets + sex + age + indication,family = binomial)
