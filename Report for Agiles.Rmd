...
AGiles$age = factor(x = AGiles$age, levels = age_levels, ordered = F)
AGiles_OG$age = factor(x = AGiles_OG$age, levels = age_levels, ordered = F)
# 
# levels(AGiles$age) <- betterLevels
# # levels(AGiles_OG$age) <- betterLevels
# contrasts(AGiles$age) <- contr.treatment(8)
# contrasts(AGiles_OG$age) <- contr.treatment(8)


countOfDef <- AGiles %>% group_by(`Default.REA.buckets`) %>%
  tally() %>%
  arrange(-n)

AGiles$`Default.REA.buckets` <- factor(AGiles$`Default.REA.buckets`, countOfDef$Default.REA.buckets)
AGiles_OG$`Default.REA.buckets` <- factor(AGiles_OG$`Default.REA.buckets`,
                                          countOfDef$Default.REA.buckets)
...
```{r reclassificationEDAPlotsDEFAULTS, include=FALSE}
### RECLASSSIFICATION plot ###
reclassMeansDef <- group_by(latestAgilesGG, `Default.REA.buckets`) %>%
  dplyr::summarise(meanReclassification = mean(Reclassified),
            uci_perGroup = CI(Reclassified)['lower'],
            lci_perGroup = CI(Reclassified)['upper']) %>%
  arrange(meanReclassification)

reanazMeansDef <- group_by(latestAgilesGG, `Default.REA.buckets`) %>%
  dplyr::summarise(meanReanaz = mean(reanalyzed),
                   uci_perGroup = CI(reanalyzed)['lower'],
                   lci_perGroup = CI(reanalyzed)['upper']) %>%
  arrange(meanReanaz)

classMeansDef <- group_by(latestAgilesGG, `Default.REA.buckets`) %>%
  dplyr::summarise(meanClassification = mean(classification),
                   uci_perGroup = CI(classification)['lower'],
                   lci_perGroup = CI(classification)['upper']) %>%
  arrange(meanClassification)

latClassMeansDef <- group_by(latestAgilesGG, `Default.REA.buckets`) %>%
  dplyr::summarise(meanClassification = mean(classification),
                   uci_perGroup = CI(classification)['lower'],
                   lci_perGroup = CI(classification)['upper']) %>%
  arrange(meanClassification)
...
reclassGGDef <- ggplot(reclassMeansDef) +
  geom_bar( aes(x=`Default.REA.buckets`, y=meanReclassification), stat = "identity", alpha = 0.5) +
  geom_errorbar( aes(x=`Default.REA.buckets`, ymin=lci_perGroup, ymax=uci_perGroup, color = `Default.REA.buckets`), alpha=0.9, size=1)+
  coord_flip()+
  ylab("mean reclass") +
  theme_dark()

reanazGGDef <- ggplot(reanazMeansDef) +
  geom_bar( aes(x=`Default.REA.buckets`, y=meanReanaz), stat = "identity", alpha = 0.5) +
  geom_errorbar( aes(x=`Default.REA.buckets`,
                     ymin=lci_perGroup, ymax=uci_perGroup, 
                     color = `Default.REA.buckets`), alpha=0.9, size=1)+
  coord_flip() +
  ylab( "mean R2") +
  theme_dark()

classGGDef <- ggplot(classMeansDef) +
  geom_bar( aes(x=`Default.REA.buckets`, y=meanClassification), stat = "identity", alpha = 0.9) +
  geom_errorbar( aes(x=`Default.REA.buckets`, ymin=lci_perGroup, ymax=uci_perGroup, color = `Default.REA.buckets`), alpha=0.9, size=1)+
  ggtitle("Cumulative classifications") +
  coord_flip()+
  theme_dark()

latClassGGDef <- ggplot(latClassMeansDef) +
  geom_bar( aes(x=`Default.REA.buckets`, y=meanClassification), stat = "identity", alpha = 0.9) +
  geom_errorbar( aes(x=`Default.REA.buckets`, ymin=lci_perGroup, ymax=uci_perGroup, color = `Default.REA.buckets`), alpha=0.9, size=1)+
  ggtitle("Up to Date classifications") +
  coord_flip()+
...
binnResid <- function(model){
  arm::binnedplot(x = predict(model), y = resid(model,type = "response"), 
                  main = paste("Binned residual plot: \n",
                               c(model$call[1:3]),
                               "\n",
                               tail(model$call, 1),
...
```{r anotherattempt, include = FALSE}
library(sjPlot)

listOfTerms <- c("Default.REA.bucketsHispanic / Latino",
                 "Default.REA.bucketsMultiracial",
                 "Default.REA.bucketsAfrican American / Black",
                 "Default.REA.bucketsAsian",
                 "Default.REA.bucketsN/A",
                 "Default.REA.bucketsAshkenazi Jewish",
                 "Default.REA.bucketsOther")

listOfLabels <- c("Hispanic and Latino",
                 "Multiracial",
...
names(listOfLabels) = listOfTerms
...
plot_model(model1.1,
           ...
           show.values = T,
           value.offset = .3,
           title = "model1.1 classification",
           axis.labels = listOfLabels,
           axis.lim = c(0.75,3))
...
```{r model1OddsRatio, echo=FALSE, message=FALSE, results='hide'}

model1OR <- plot_model(model1.1,
           terms = listOfTerms,
           sort.est = T,
           vline.color = "red",
           show.values = T,
...
suppressWarnings(print(model1OR))
...
```{r model12OddsRatio, echo=FALSE, message=FALSE, results='hide'}

model12OR <- plot_model(model1.2,
           terms = listOfTerms,
           sort.est = T,
           vline.color = "red",
           show.values = T,
...
suppressWarnings(print(model12OR))
...
```{r model2OddsRatio, echo=FALSE, message=FALSE, results='hide'}

model2OR <- plot_model(model2,
           terms = listOfTerms,
           sort.est = T,
           vline.color = "red",
           show.values = T,
...
suppressWarnings(print(model2OR))
```
#### `r nameOfOutput.4`
...
```{r model22OddsRatio, echo=FALSE, message=FALSE, results='hide'}

reclassOR <- plot_model(reclass22,
           terms = listOfTerms,
           sort.est = T,
           vline.color = "red",
           show.values = T,
...
suppressWarnings(print(reclassOR))
```
...
```{r outcomes62OddsRatio, message=FALSE, results='hide'}
outcomeOR <- plot_model(outcomes62,
           terms = listOfTerms,
           sort.est = T,
           vline.color = "red",
           show.values = T,
           value.offset = .3,
           title = "outcome62 classification",
           axis.labels = listOfLabels,
           axis.lim = c(0.75,3))
           #transform = "plogis")

suppressWarnings(print(outcomeOR))
```
...
```{r effectsVisualization, include = FALSE, eval = FALSE}
library(effects)

plot(allEffects(model3.1))
plot(predictorEffect("abcBuckets", model3.1))
summary(model3.1)

abcEffect <- predictorEffect("abcBuckets", model3.1)

# https://cran.r-project.org/web/packages/effects/vignettes/predictor-effects-gallery.pdf
or <- order(as.data.frame(abcEffect)$fit) # order smallest to largest
originals$abcBuckets2 <- factor(originals$abcBuckets, # reorder levels of spp
  levels=levels(originals$abcBuckets)[or])
model3.1.effect <- update(model3.1, ~ . - abcBuckets + abcBuckets2) # refit model
plot(predictorEffects(model3.1.effect, ~ abcBuckets2), main="type='response', ordered",
  axes=list(y=list(type="response",
  lab="probabilty scale, probability labels"),
  x=list(rotate=30, spp=list(lab="abc Buckets")),
  grid=TRUE))

defEffect <- predictorEffect("Default.REA.buckets", model1.1)

# https://cran.r-project.org/web/packages/effects/vignettes/predictor-effects-gallery.pdf
or <- order(as.data.frame(defEffect)$fit) # order smallest to largest
originals$def2 <- factor(originals$`Default.REA.buckets`, # reorder levels of spp
  levels=levels(originals$`Default.REA.buckets`)[or])
model1.1.effect <- update(model1.1, ~ . - `Default.REA.buckets` + def2) # refit model
plot(predictorEffects(model1.1.effect, ~ def2), main="type='response', ordered",
  axes=list(y=list(type="response",
  lab="probabilty scale, probability labels"),
  x=list(rotate=30, spp=list(lab="default Buckets")),
  grid=TRUE))


# plotOrderedEffects <- function(model,eff.str){
#   abcEffect <- predictorEffect(eff.str, model)
# 
# # https://cran.r-project.org/web/packages/effects/vignettes/predictor-effects-gallery.pdf
# or <- order(as.data.frame(abcEffect)$fit) # order smallest to largest
# # how do you specify to use a cusotom predictor selector...?
# model$data$newEff <- factor(model$data$eff.str, # reorder levels of spp
#   levels=levels(model$data$eff.str)[or])
# model.effect <- update(model, ~ . - as.name(eff.str) + newEff) # refit model
# plot(predictorEffects(model.effect, ~ newEff), main="type='response', ordered",
#   axes=list(y=list(type="response",
#   lab="probabilty scale, probability labels"),
#   x=list(rotate=30, spp=list(lab="abc Buckets")),
#   grid=TRUE))
...
# }
# 
# plotOrderedEffects(model1.1,"Default.REA.buckets")

```



```{r usingChatGPT, eval=FALSE, include=FALSE}
plotOdds <- function(model) {
...
plot(coef, exp(coef), pch = 19, xlim = c(-1, 1), ylim = c(0, 5))
arrows(x0 = coef, y0 = ci_low, x1 = coef, y1 = ci_high, length = 0.05, angle = 90, code = 3)
abline(h = 1, lty = 2)
axis(1, at = coef, labels = names(coef))

}

plotOdds(model1.1)

plot(predictorEffect("abcBuckets", model3.1), scales = list(x = list(at = 1:8, labels = 1:8)), 
     ylab = "Odds Ratio", ylim = c(0, 5), main = "Effects of x on the response variable")

# create a dataset for logistic regression
plotOdds2 <- function(model, var){
# create a dataframe with odds ratios and confidence intervals for the categorical variable
coef_df <- data.frame(coef = names(coef(model)),
                      odds_ratio = exp(coef(model)),
                      ci_low = exp(confint.default(model)[,1]),
                      ci_high = exp(confint.default(model)[,2]))

# filter the dataframe to include only the odds ratio for one category
coef_df <- coef_df[coef_df$coef == var,]

# plot the odds ratio with confidence intervals
ggplot(coef_df, aes(x = coef, y = odds_ratio)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.2) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  ylab("Odds Ratio") +
  ggtitle(paste("Odds Ratio for Category",var))
}
```
...
```{r include = FALSE}
# model1.1 model1.2 model2 reclass22

# export the plot to a .png file
png(file = "ORmodel11.png", width = 800, height = 600, res = 150)
...
# export the plot to a .png file
png(file = "ORmodel12.png", width = 800, height = 600, res = 150)
...
# export the plot to a .png file
png(file = "ORmodel2.png", width = 800, height = 600, res = 150)
...
# export the plot to a .png file
png(file = "ORreclass22.png", width = 800, height = 600, res = 150)
